    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead className="border-b border-[#e6edf0] bg-[#f1f5f7]">
          <tr>{["Applicant", "Course", "Progress", "Callback", "Payment", "Next Call", ""].map((heading) => <th key={heading} className="whitespace-nowrap px-5 py-3 text-left text-[11px] font-bold uppercase tracking-wide text-[#010608]/45">{heading}</th>)}</tr>
        </thead>
        <tbody>
          {applications.map((application) => (
            <tr key={application.id} className="border-b border-[#e6edf0] bg-white transition hover:bg-[#f8fafb]">
              ...
              <td className="whitespace-nowrap px-5 py-4 text-xs text-[#010608]/55">{application.nextCallbackAt ? application.nextCallbackAt.toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" }) : "—"}</td>
              <td className="px-5 py-4"><Link href={`/admin/applications/${application.id}`} className="inline-flex items-center gap-1 whitespace-nowrap text-xs font-bold text-[#04415f] hover:text-[#2086b8]">Open <ArrowUpRight size={13} /></Link></td>
            </tr>
          ))}
        </tbody>
      </table>
