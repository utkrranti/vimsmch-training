        ...
        <tbody>
          {applications.map((application) => (
            <tr key={application.id} className="border-b border-[#e6edf0] bg-white transition hover:bg-[#f8fafb]">
              <td className="px-5 py-4"><p className="font-semibold text-[#011e2c]">{application.name}</p><p className="mt-0.5 text-xs text-[#010608]/45">{application.applicationNo}</p><a href={`tel:${application.phone}`} className="mt-1 inline-flex items-center gap-1 text-xs font-medium text-[#2086b8]"><PhoneCall size={11} />{application.phone}</a><p className="mt-1 text-[10px] text-[#010608]/40">{application.filledByAdminName ? `Filled by staff: ${application.filledByAdminName}` : "Filled by applicant (self)"}</p></td>
