...
import { ClipboardCheck, PhoneCall, TimerReset, UserCheck, FileSpreadsheet, UserPlus } from "lucide-react";
...
export default async function ApplicationsPage({ searchParams }: PageProps) {
  ...
  return <div className="space-y-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="flex items-center gap-2 text-2xl font-bold text-[#011e2c]"><ClipboardCheck size={23} className="text-[#04415f]" /> Admission Applications</h1><p className="mt-1 text-sm text-[#010608]/45">Track applicants, callbacks, documents, and payments.</p></div><div className="flex flex-wrap gap-2 shrink-0"><Link href="/admin/applications/new" className="flex items-center gap-2 rounded-xl border border-[#04415f] px-4 py-2.5 text-sm font-bold text-[#04415f] transition hover:bg-[#edf8fc]"><UserPlus size={16} /> New Application</Link><a href="/api/admin/applications/export" className="flex items-center gap-2 rounded-xl bg-[#04415f] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[#065a82]"><FileSpreadsheet size={16} /> Export to Excel</a></div></div><div className="grid gap-4 sm:grid-cols-3"><Summary icon={PhoneCall} label="Total leads" value={total} /><Summary icon={TimerReset} label="Callbacks due" value={dueCallbacks} /><Summary icon={UserCheck} label="Submitted" value={submitted} /></div><div className="flex flex-wrap gap-2">{["", "DRAFT", "SUBMITTED", "DOCUMENT_VERIFICATION", "CORRECTION_REQUIRED", "ENROLLED", "REJECTED"].map((item) => <a key={item || "ALL"} href={item ? `/admin/applications?status=${item}` : "/admin/applications"} className={`rounded-full px-4 py-2 text-xs font-semibold ${status === item || (!status && !item) ? "bg-[#04415f] text-white" : "border border-[#e2eaee] bg-white text-[#010608]/55"}`}>{item ? item.replaceAll("_", " ") : "ALL"} <span className="ml-1 opacity-65">{item ? counts[item] ?? 0 : total}</span></a>)}</div><div className="overflow-hidden rounded-2xl border border-[#e2eaee] bg-white shadow-sm"><ApplicationTable applications={applications} /></div></div>;
}
